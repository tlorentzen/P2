...
namespace FileSplitter{
    public class splitterLibary{
        ...
        public void splitFile(String inputFilePath, string OutputFolderpath, int chunkSize){
            ...
            if (File.Exists(inputFilePath)){
                ...
                using (Stream input = File.OpenRead(inputFilePath)){
                    ...
                    while (input.Position < input.Length){
                        string filehashName = fileHash + rnd.Next(1, 8000);
        ...
        public void mergeFiles(string inputDir, string outputFilePath){
            if (Directory.Exists(inputDir)){
                if (_files.Count > 0){
                    ...
                    using (Stream output = File.OpenWrite(outputFilePath)){
                        foreach (String chunkPath in _files){
